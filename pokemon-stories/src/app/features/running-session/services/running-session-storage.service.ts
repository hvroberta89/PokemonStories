...
export class RunningSessionStorageService {
  ...
  private readonly storageKey =
    'pokemon-stories.running-session';
  ...
  save(
    ...
    try {
      ...
      console.error(
        'A Running Session mentése sikertelen.',
        error,
      );
    }
  }
  ...
  private isRunningSessionState(
    value: unknown,
  ): value is RunningSessionState {
    if (
      typeof value !== 'object' ||
      value === null
    ) {
    ...
    return (
      ...
      typeof candidate.viewModel ===
        'object' &&
      candidate.viewModel !== null &&
      Array.isArray(
        candidate.rewardQueue,
      ) &&
      Array.isArray(
        candidate.rewardHistory,
      )
    );
  }
}